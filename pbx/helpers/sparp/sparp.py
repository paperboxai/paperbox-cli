...
import time
from typing import Callable, Dict, List, Iterator, Union
...
async def consumer(
    source_queue, source_semaphore, sink_queue, session, shared, ok_status_codes, stop_on_first_fail
):
    while True:
        ...
        if isinstance(config, dict):
            ...
            response = await session.request(**(await config()))
...
async def async_main(
    ...
    retry_options = ExponentialRetry(
        attempts=retry_attempts, statuses=set(retry_status_codes), retry_all_server_errors=False
    )
...
def sparp(
    configs: Iterator[Union[Dict, Callable]],
    ...
    Arguments:
      configs (List[Union[Dict, Callable]]): the request configurations. Each item in this list is fed roughly as such: [requests.request(**config) for config in configs]
